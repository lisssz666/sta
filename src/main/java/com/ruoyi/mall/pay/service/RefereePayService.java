...
public class RefereePayService {
    ...
    @Value("${wechat.referee_pay.notify_url}")
    ...
    public Map<String, String> getPaymentParams(String code, String amount, String outTradeNo, String description) throws Exception {
        ...
        System.out.println("裁判支付获取openid成功：" + openid);

        // 2. 获取prepay_id
        String prepayId = getPrepayId(openid, amount, outTradeNo, description);
        System.out.println("裁判支付获取prepay_id成功：" + prepayId);
    ...
    private String getOpenidByCode(String code) throws Exception {
        // 1. 先从Redis缓存中查找
        String cacheKey = "referee:code:" + code;
    ...
    private Map<String, String> generatePaymentParams(String prepayId) throws Exception {
        ...
        System.out.println("裁判支付参数生成成功");
        return result;
    }

    @Autowired
    private com.ruoyi.mall.order.service.RefereeOrderService refereeOrderService;
    ...
    public void handlePayNotify(String notifyData) throws Exception {
        System.out.println("开始处理裁判支付通知：" + notifyData);
        ...
        if (resource != null) {
            ...
            // 更新裁判订单状态为已支付
            if (outTradeNo.startsWith("RF")) {
                // 由于RefereeOrderService接口没有提供lambdaQuery方法，我们需要通过实现类来访问
                QueryWrapper<RefereeOrder> queryWrapper = new QueryWrapper<>();
                queryWrapper.eq("order_no", outTradeNo);
                
                // 假设refereeOrderService是RefereeOrderServiceImpl的实例，我们可以通过类型转换来访问ServiceImpl的方法
                if (refereeOrderService instanceof IService) {
                    IService<RefereeOrder> service = (IService<RefereeOrder>) refereeOrderService;
                    RefereeOrder refereeOrder = service.getOne(queryWrapper);
                    
                    if (refereeOrder != null) {
                        refereeOrder.setStatus(1); // 1已付款
                        refereeOrder.setTransactionId(transactionId);
                        refereeOrder.setPayTime(java.time.LocalDateTime.now());
                        service.updateById(refereeOrder);
                        System.out.println("裁判订单状态更新成功：" + outTradeNo);
                    }
                }
            }
        } else {
            System.out.println("裁判支付通知处理失败：缺少resource字段");
     ...
     * 构建微信支付请求体
     * <p>
     * 根据支付参数构建符合微信支付API要求的请求体
     * 格式化金额、设置过期时间、配置通知地址等
     * </p>
     */
     ...
     * 构建Authorization头
     * <p>
     * 生成符合微信支付API要求的Authorization请求头
     * 使用RSA2048算法进行签名
     * </p>
     */
